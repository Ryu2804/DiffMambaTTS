...
from f5_tts.model import CFM, DiT, Mamba3Backbone, Trainer, UNetT
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["F5TTS_v1_Base", "F5TTS_Base", "E2TTS_Base", "Mamba3TTS_Base"],
...
def main():
    ...
    if args.exp_name == "F5TTS_v1_Base":
    ...
    elif args.exp_name == "Mamba3TTS_Base":
        wandb_resume_id = None
        model_cls = Mamba3Backbone
        model_cfg = dict(
            dim=1024,
            depth=22,
            ff_mult=2,
        ...
        if args.finetune:
            if args.pretrain is None:
                raise ValueError(
                    "--pretrain must be specified when fine-tuning Mamba3TTS_Base "
                    "(no public checkpoint available yet)."
                )
            ckpt_path = args.pretrain
